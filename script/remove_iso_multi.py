...
def filter_gbk_preserve_origin_line(input_gbk, output_gbk):
    ...
    for rec_idx, record in enumerate(SeqIO.parse(input_gbk, "genbank")):
        ...
        # ── Clean up contig name: remove underscores from all relevant fields ───
        # ── Clean underscores using Biopython-safe method ───────────────
        ...
        if "sequence_version" in record.annotations:
            ...
            if isinstance(version, str):
                ...
                version = version  # leave unchanged (usually doesn’t contain _)
            record.annotations["sequence_version"] = version


        cleaned_records.append(record)
        kept_locus_lines.append(original_locus_lines[rec_idx].replace("_", ""))
