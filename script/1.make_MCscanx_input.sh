...
#make strain_taxo from selected_gbff
strain_taxo_path=$(cd "$output/.." && pwd)
# Header
echo -e "class\torder\tfamily\tgenus\tspecies" > $strain_taxo_path/strain_taxo
awk '{print $NF}' "$output/selected_gbff" > "$output/species"
if [[ -n "${input_aug:-}" ]]; then
...
while read  B; do
    ...
    # Output line
    printf "%s\t%s\t%s\t%s\t%s\n" "$class" "$order" "$family" "$genus" "$B" >> $strain_taxo_path/strain_taxo
done < "$output/species"
