...
if [[ -f "${input_aug:-}" ]]; then
  ...
  gff_2_gbk() {
    ...
    python "$BIN_PATH/script/remove_iso_multi.py" "$output/$organism.gbk" "$output/orign_gbk/${organism}.gbk"
...
' >> "$output/check_gbff"
  ...
  form_gbk () {
    ...
    python "$BIN_PATH/script/remove_iso_multi.py" "$gbff" "$output/${organism}.gbk"
    ...
    if [[ -s "$output/${organism}.gbk" ]]; then
      ...
      # Clean up temp files
      #rm "$output/${organism}.gff" "$output/${organism}.namedic" "$output/${organism}.gbk"
    else
