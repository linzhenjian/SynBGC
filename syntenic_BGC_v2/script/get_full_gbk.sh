...
while read i; do
	...
	awk -F _ '{print $NF}' V1_output/collinear/processed/$id.txt > temp_id
	if [ ! -s ${strain}_${contig}.gbk ]; then
		python ~/../schmidt-group3/software/syntenic_BGC_v2/script/extract_contig_gbk.py  ${strain}.anno.GBK ${strain}_${contig}.gbk $contig
	else
